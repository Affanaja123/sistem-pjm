...
export const Route = createFileRoute('/_adminkonten/admin-konten/sistem-informasi')({
...
function RouteComponent() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/get_sistem_info.php`)
      .then(res => res.json())
      .then(res => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/save_sistem_info.php`, {
        method: 'POST',
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto space-y-6 min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Editor Sistem Informasi</h1>
          <p className="text-slate-500 text-sm mt-1">Konfigurasi konten untuk halaman Sistem Informasi.</p>
        </div>
          ...
          <div className="space-y-1.5">
            ...
            {!data.bgImage ? (
              <div 
                ...
                onClick={() => document.getElementById('sistem-upload')?.click()}
              >
            ...
            <input type="file" id="sistem-upload" className="hidden" accept="image/*" onChange={handleImageChange} />
          </div>
