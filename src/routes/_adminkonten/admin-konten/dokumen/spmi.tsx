...
export const Route = createFileRoute('/_adminkonten/admin-konten/dokumen/spmi')({
  component: SpmiEditor,
});
...
function SpmiEditor() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/get_spmi.php`)
      .then(res => res.json())
      .then(res => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/save_spmi.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      });
      const result = await response.json();
      alert(result.status === 'success' ? "Berhasil disimpan!" : "Gagal: " + result.message);
    } catch (err) { alert("Error saat menyimpan."); } finally { setSaving(false); }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-8 bg-slate-50/50 min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Perangkat SPMI</h1>
          <p className="text-slate-500 mt-1">Kelola dokumen Sistem Penjaminan Mutu Internal.</p>
        </div>
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between items-center">
            <CardTitle>Daftar Perangkat SPMI</CardTitle>
        ...
        <CardContent className="space-y-6">
          {data.documents.map((doc, docIndex) => (
            <div key={doc.id} className="border p-4 rounded-lg space-y-4 bg-white shadow-sm">
              <div className="flex gap-4">
                <FileText className="mt-2 h-5 w-5 text-slate-400 shrink-0" />
              ...
              <div className="pl-9 border-t pt-4 space-y-2">
                <div className="flex justify-between items-center">
                    <Label className="text-[10px] font-bold uppercase text-slate-500">Daftar Dokumen</Label>
