...
export const Route = createFileRoute('/_adminkonten/admin-konten/konsultasi-mutu')({
...
function RouteComponent() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/get_konsultasi.php`)
      .then(res => res.json())
      .then(res => {
        if (res.status === 'success' && res.data) {
          setData({
            ...
            bgImage: res.data.image_path ? `${API_BASE_URL}/uploads/${res.data.image_path}` : null,
          });
        }
        setLoading(false);
      })
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/save_konsultasi.php`, {
        method: 'POST',
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto space-y-6 min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Editor Halaman Konsultasi</h1>
          <p className="text-slate-500 text-sm mt-1">Konfigurasi konten untuk halaman Konsultasi Mutu.</p>
        </div>
          ...
          <div className="space-y-1.5">
            ...
            {!data.bgImage ? (
              <div 
                ...
                onClick={() => document.getElementById('konsultasi-upload')?.click()}
              >
            ...
            <input type="file" id="konsultasi-upload" className="hidden" accept="image/*" onChange={handleImageChange} />
          </div>
