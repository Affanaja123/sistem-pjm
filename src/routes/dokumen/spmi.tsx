...
export const Route = createFileRoute('/dokumen/spmi')({
    loader: async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/get_spmi.php`);
            return await res.json();
        } catch (e) { return null; }
    },
    component: PerangkatSPMIPage,
});

function PerangkatSPMIPage() {
    const loaderData = Route.useLoaderData();
    // Inisialisasi state dari Loader data atau LocalStorage
    const [data, setData] = useState(() => {
        if (loaderData?.status === 'success') {
            localStorage.setItem('spmi_cache', JSON.stringify(loaderData.data));
            return loaderData.data;
        }
        const cached = localStorage.getItem('spmi_cache');
        return cached ? JSON.parse(cached) : { 
            title: "PERANGKAT SPMI", 
            subtitle: "Sistem Penjaminan Mutu Internal", 
            bg_url: null, 
            documents: [] 
        };
    });
    ...
    const [openSection, setOpenSection] = useState<string | null>(
        data.documents.length > 0 ? data.documents[0].name : null
    );
                    ...
                    {data.documents.map((doc: any, index: number) => (
                        <div
                            ...
                            className={`bg-white rounded-lg border transition-all duration-300 ${
                                openSection === doc.name
                                    ? 'border-[#367fa9] shadow-sm'
                                    : 'border-[#367fa9]/30 hover:border-[#367fa9]'
                            }`}
                        >
                            <button
                                onClick={() => setOpenSection(openSection === doc.name ? null : doc.name)}
                                className="w-full flex justify-between items-center p-8 text-left"
                            >
                                <div className="flex items-center gap-6">
                                    <div className={`w-1 h-8 rounded-full ${openSection === doc.name ? 'bg-[#367fa9]' : 'bg-[#367fa9]/30'}`} />
                                    <div>
                                        <span className="block text-lg font-bold text-gray-900 uppercase tracking-widest">{doc.name}</span>
                                        <p className="text-sm text-gray-500 mt-1 font-light">{doc.description}</p>
                                    </div>
                                </div>

                                <svg
                                    className={`w-5 h-5 text-[#367fa9] transform transition-transform duration-300 ${openSection === doc.name ? 'rotate-180' : ''}`}
